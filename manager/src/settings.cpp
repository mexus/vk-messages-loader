...
void JsonToObject<manager::Settings>(const rapidjson::Value& json, manager::Settings* object) {
    JsonGetMembers(json,
                   ...
                   "users", &object->users_, util::json::Optional{});
}
...
std::string Settings::GetApplicationId() const {
    return application_id_;
}

void Settings::SetApplicationId(const std::string& id) {
    application_id_ = id;
}

std::string Settings::GetStoragePath() const {
    return storage_path_;
}

void Settings::SetStoragePath(const std::string& path) {
    storage_path_ = path;
}

const std::vector<uint64_t>& Settings::GetUsers() const {
    return users_;
}

std::vector<uint64_t>& Settings::GetUsers() {
    return users_;
}
