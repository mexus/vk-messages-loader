...
HistoryStorage::HistoryStorage(const std::string& path) : path_(path) {
}

std::shared_ptr<Substorage> HistoryStorage::GetUser(uint64_t user_id) {
...
std::shared_ptr<Substorage> HistoryStorage::GetStorage(uint64_t user_id) {
    if (!CheckPath()) {
        return {};
    }
    ...
    auto storage = new Substorage(path.string<std::string>());
    return std::shared_ptr<Substorage>(storage);
}

bool HistoryStorage::CheckPath() const {
    boost::filesystem::path path(path_);
    if (boost::filesystem::exists(path)) {
        ...
        try {
            ...
            std::cerr << "Unable to crate path {" << path << "} because of an error: " << fs_error.what() << "\n";
            return false;
        }
        if (!boost::filesystem::exists(path)) {
...
bool HistoryStorage::CheckFile(const std::string& path) {
