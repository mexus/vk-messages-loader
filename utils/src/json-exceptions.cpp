...
std::string TypeException::GetType() const { return type_; }

NoFieldException::NoFieldException(const std::string& at,
                                   const std::string& field)
    : Exception(at, "no field '" + field + "' in a json object"),
      field_(field) {}

std::string NoFieldException::GetField() const { return field_; }
