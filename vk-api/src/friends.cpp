...
std::vector<User> FriendsAPI::GetFriends() {
  const cpr::Parameters const_params{{"fields", "uid,first_name,last_name"},
                                     {"order", "hints"}};
  ...
  while (true) {
    ...
    auto doc =
        vk_interface_->SendRequest(kInterfaceName, "get", std::move(params));

    List<User> response;
    try {
      util::JsonGetMember(doc, "response", &response);
    } catch (const util::json::Exception& e) {
      LOG(ERROR) << "Unable to convert response to a list of friends: "
                 << e.what();
      break;
    }
